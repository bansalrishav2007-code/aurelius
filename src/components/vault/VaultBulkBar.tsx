...
export function VaultBulkBar({ count, onClear, onDownload, onDelete, onMove, busy }: Props) {
  if (count === 0) return null;

  return (
    <div className="sticky top-0 z-20 mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-[#c9a84c]/30 bg-[#0a0e1a]/95 px-4 py-3 backdrop-blur-sm">
      <span className="text-xs text-[#c9a84c] font-medium">{count} selected</span>
      <button
        type="button"
        disabled={busy}
        onClick={onDownload}
        className="h-8 px-3 rounded-lg border border-border/50 text-xs inline-flex items-center gap-1.5 hover:border-[#c9a84c]/40 disabled:opacity-50"
      >
